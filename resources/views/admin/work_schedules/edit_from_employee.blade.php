...
@section('content')
<h3 class="card-title">Editar escalas de <i>{{ $employee->name }}</i></h3>
        ...
        <div class="tab-content" id="nav-tabContent">         
            ...
            <div class="tab-pane fade" id="nav-{{ $value }}" role="tabpanel" aria-labelledby="nav-{{ $value }}-tab">
                <div class="card custom card-body">     
                    <form action="{{ action('Admin\WorkSchedulesController@updateFromEmployee', ['id' => $employee->id]) }}">
                        {{ csrf_field() }}
                        <input type="hidden" name="employee_id" value="{{ $employee->id }}" >
                        @foreach($employee->work_schedules as $workschedule)
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" id="workschedule-building" name="buildings_id[]" required>
                                                ...
                                                @foreach($buildings as $building)
                                                    <option value="{{ $building->id }}" @if($workschedule->building_id == $building->id) selected @endif>{{ $building->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                        ...
                        @endforeach
                        <br />
                        <a role="button" class="btn btn-danger" href="{{ action('Admin\EmployeesController@view', ['registration_number' => $employee->registration_number]) }}">Cancelar</a>
